...
$event_start_datetime = tribe_get_start_date( $post->ID, true, 'Y-m-d H:i:s' );
$event_access = (get_post_meta( $post->ID, 'studio_access', true) > 0) ? get_post_meta( $post->ID, 'studio_access', true) : 15 ;
$event_start_seconds = strtotime($event_start_datetime) - ($event_access * 60);
$studio_access_time = date("g:i a", $event_start_seconds);
$event_end_datetime = tribe_get_end_date( $post->ID, true, 'Y-m-d H:i:s' );
$event_egress = (get_post_meta( $post->ID, 'studio_egress', true) > 0) ? get_post_meta( $post->ID, 'studio_egress', true) : 15 ;
$event_end_seconds = strtotime($event_end_datetime) + ($event_egress * 60);
$studio_egress_time = date("g:i a", $event_end_seconds);
$payment_info = _cmk_get_payment_status($post->ID);
...
<div id="tribe-events-event-<?php echo esc_attr( $event_id ); ?>" class="<?php tribe_events_event_classes() ?>" data-tribejson='<?php echo esc_attr( tribe_events_template_data( $post ) ); ?>'>
	...
	<ul class="cmk-event-details">
		<li>Access: <?php echo $studio_access_time;?></li>
		<li>Egress: <?php echo $studio_egress_time; ?> </li>
		<li>Payment status: <?php echo $payment_info; ?></li>
	</ul>
